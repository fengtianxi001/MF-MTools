...
export default {
	...
	data() {
		return {
			title: this.mode === 'add' ? '添加程序' : '编辑程序信息',
			form: {
				category: null,
				icon: null,
				id: null,
	...
	methods: {
		...mapMutations(['addSoftware', 'updateSoftware']),
		submit() {
		...
		async unshiftFile() {
			...
			const configPath = this.$store.getters.config.softSavePath
			...
			// 		await File.copy(originPath, targetPath)
			// 	}
			// }
			this.addSoftware(this.form)
			loading.close()
		},
		modifyFile() {
			this.updateSoftware(this.form)
		},
		// nameCheck(name) {
